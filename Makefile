...
HEALTHCHECK-MAIN:=$(CURDIR)/cmd/healthcheck
HEALTHCHECK-BIN:=$(CURDIR)/bin/healthcheck

.PHONY: build-healthcheck
build-healthcheck: go-deps
	$(info building 'healthcheck' server...)
	@go build -ldflags="$(LDFLAGS)" -o $(HEALTHCHECK-BIN) $(HEALTHCHECK-MAIN)

.PHONY: build-healthcheck-d
build-healthcheck-d:
	$(info building 'healthcheck-debug' server...)
	@go build -ldflags="$(LDFLAGS)" -gcflags="all=-N -l" -o $(HEALTHCHECK-BIN)-dbg $(HEALTHCHECK-MAIN)
